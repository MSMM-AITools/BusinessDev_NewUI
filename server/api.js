...
app.post('/api/proposals', async (req, res) => {
    try {
        ...
        const columns = Object.keys(data).filter(key => key !== 'CLIENT_NAME'); // Exclude joined columns
        ...
        // Clean up data - convert empty date strings to null
        const cleanData = { ...data };
        dateColumns.forEach(dateCol => {
            if (cleanData[dateCol] === '') {
                cleanData[dateCol] = null;
            }
        });
...
app.post('/api/proposals/:id/archive-and-copy-to-proposal', async (req, res) => {
    try {
        ...
        const columns = Object.keys(pursuit).filter(key =>
            key !== 'PID' && key !== 'CLIENT_NAME'
        );

        const newData = { ...pursuit };
